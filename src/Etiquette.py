        ...
        for i in range(3):
            ...
            for j in range(3):
                ...
                if((row != self.row or column != self.column) and row >= 0 and row < self.minesweeper.height and column >= 0 and column < self.minesweeper.width):
                    if not self.minesweeper.is_clicked(row, column) and self.minesweeper.is_flagged(row, column):
                        nbrFlags += 1
        if nbrFlags == self.number:
            for i in range(3):
                ...
                for j in range(3):
                    column = self.column + j - 1
                    if((row != self.row or column != self.column) and row >= 0 and row < self.minesweeper.height and column >= 0 and column < self.minesweeper.width):
                        if not self.minesweeper.is_clicked(row, column) and not self.minesweeper.is_flagged(row, column):
